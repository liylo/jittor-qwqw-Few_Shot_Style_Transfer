    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                x = x.to(lora_A.dtype)
                result = result + (dropout(x) @ (lora_A * lora_E).t() @ lora_B.t()) * scaling / ranknum
...
class RankAllocator:
        ...
        for n, p in model.named_parameters():
            if "lora_" in n and self.adapter_name in n:
                if p.grad is not None:
                    self.paramdict[n] = p.grad.clone().detach()
            ...
            if "lora_" in n and self.adapter_name in n:
                ...
                with torch.no_grad():
                    if deepspeed_config() is not None:
                        ...
                        self.ipt[n] = (p.detach() * self.paramdict[n]).abs().detach()
    ...
    def mask_to_budget(self, model, budget):
        ...
        k=max(-(torch.cat(all_score).shape[0])+1,self.init_bgt - budget)
