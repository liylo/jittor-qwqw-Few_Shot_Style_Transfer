...
class OFTLayer(nn.Module, LycorisLayer):
    ...
    def _available_adapters(self) -> Set[str]:
        return {*list(self.oft_r.keys())}

    def create_adapter_parameters(self, adapter_name: str, r: int, shape: Tuple[int, ...], block_share: bool):
        if block_share:
            self.oft_r[adapter_name] = nn.Parameter(torch.empty((1, math.ceil(shape[0] / r), math.ceil(shape[0] / r)),requires_grad=True))
        else:
            self.oft_r[adapter_name] = nn.Parameter(torch.empty((r, math.ceil(shape[0] / r), math.ceil(shape[0] / r)),requires_grad=True))
    ...
    def get_delta_weight(self, adapter_name: str) -> torch.Tensor:
        ...
        if coft:
            with torch.no_grad():
                opt_r=(self._project_batch(opt_r, eps=eps)).copy()
    ...
    def _cayley_batch(self, data: torch.Tensor) -> torch.Tensor:
        ...
        diag = jt.ones(r)
        I = jt.diag(diag).unsqueeze(0).expand(b, r, c)

        # Perform the Cayley parametrization
        Q = torch.bmm(I - skew, jt.linalg.inv(I + skew))
