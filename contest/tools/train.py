...
import transformers
from peft import LoraConfig,OFTConfig,OFTModel,LoraModel
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--lora_type", type=str, default="dora", help="The lora_type.")
    parser.add_argument("--r", type=int, default=8, help="The rank of the LoRA update matrices.")
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        # get names of images in the instance data root as a new list
        # only get names instead of whole path
        self.images_names = [
            (str(path).split("/")[-1].replace(".png", "")).replace("_"," ")
        ...
        self.image_transforms = transform.Compose(
            [
                ...
                (
                    ...
                    if center_crop
                    else transform.RandomCrop(size)
                ),
                transform.ToTensor(),
    ...
    def __getitem__(self, index):
        ...
        if self.encoder_hidden_states is not None:
            ...
            # get name
            instance_image_name = self.images_names[index % self.num_instance_images]

            prompt = f"a {instance_image_name} in the style of {self.instance_prompt}"
...
def main(args):
    ...
    with open(f'{args.output_dir}/config.yaml', 'w') as config_file:
        yaml.dump(args_dict, config_file, default_flow_style=False)
    ...
    number=args.instance_prompt
    args.instance_prompt = style_dict[args.instance_prompt]
    ...
    # Train!
    total_batch_size = (
        args.train_batch_size * args.num_process * args.gradient_accumulation_steps
    )
    ...
    jt.save(state_dict, f"{args.output_dir}/unet.pth")
