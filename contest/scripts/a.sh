...
for ((folder_number = 0; folder_number <= $MAX_NUM; folder_number+=$GPU_COUNT)); do
    for ((gpu_id = 0; gpu_id < GPU_COUNT; gpu_id+=1)); do
        ...
        COMMAND="CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES python tools/run.py --seed $SEED --use_id --input_root $INPUT --rank $current_folder_number --weight $WEIGHT --output $OUTPUT"
        eval $COMMAND &
        sleep 10
    done
    wait
